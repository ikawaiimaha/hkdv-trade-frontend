...
comment on function public.mark_my_notification_read(uuid) is 'Marks one authenticated-trader notification as read.';
comment on function public.track_trade_event(text, jsonb) is 'Records a trade funnel event for the authenticated trader.';
...
revoke execute on function public.track_trade_event(text, jsonb) from public;
...
begin
  if exists (select 1 from pg_roles where rolname = 'anon') then
    ...
    revoke execute on function public.track_trade_event(text, jsonb) from anon;
  ...
  if exists (select 1 from pg_roles where rolname = 'authenticated') then
    ...
    revoke execute on function public.track_trade_event(text, jsonb) from authenticated;
