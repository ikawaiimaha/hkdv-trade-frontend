...
function mapNotification(entry) {
  return {
    ...
    title: entry.title,
    body: entry.body ?? "",
    sellerTraderId: entry.seller_trader_id ?? null,
    buyerTraderId: entry.buyer_trader_id ?? null,
    listingId: entry.listing_id ?? null,
    itemId: entry.item_id ?? null,
    offeredItemId: entry.offered_item_id ?? null,
    requestedItemId: entry.requested_item_id ?? null,
    priority: entry.priority ?? null,
    matchScore: entry.match_score == null ? null : Number(entry.match_score),
    fairnessScore: entry.fairness_score == null ? null : Number(entry.fairness_score),
    actionPayload: entry.action_payload ?? {},
    sellerUsername: entry.seller_username ?? "",
    offeredItemName: entry.offered_item_name ?? "",
    requestedItemName: entry.requested_item_name ?? "",
...
export async function loadTraderWorkspaceWithSource() {
  const [payload, birthdayRows] = await Promise.all([
  ...
  return {
    ...
    workspace: {
      ...workspace,
      currentTrader: mergeBirthdayIdentity(workspace.currentTrader, birthdayRows),
    },
  };
}
...
export async function markWorkspaceNotificationRead(notificationId) {
  return requestSupabaseRpc("mark_my_notification_read", {
    p_notification_id: notificationId,
  });
}

export async function trackTradeEvent(eventType, metadata = {}) {
  return requestSupabaseRpc("track_trade_event", {
