...
import AssetManagerSection from "./sections/AssetManagerSection";
import CatalogSection from "./sections/CatalogSection";
import DreamBoardSection from "./sections/DreamBoardSection";
    ...
    <>
      ...
      >
        ...
        <div className="sidebar-foot surface-dark">
          ...
          {isCompactShell && (canInstall || onSignOut) ? (
            <div className="button-row compact sidebar-utility-actions">
              {canInstall ? (
        ...
        <div className="topbar-head">
          <div className="topbar-brandline">
            ...
            <a className="topbar-brand" href="#dashboard">
              <img src="/hkdv-redesign/mascot-idle.png" alt="" loading="lazy" decoding="async" />
              <span>HKDV Trade</span>
            </a>
          </div>
          <div className="topbar-copy-wrap">
            <p className="eyebrow">{isCompactShell ? "Trading desk" : "Marketplace"}</p>
            <h2>{activeViewMeta.label}</h2>
            {!isCompactShell ? <p className="topbar-copy">{activeViewMeta.description}</p> : null}
          </div>
          {!isCompactShell ? (
            <nav className="topbar-nav" aria-label="Primary">
              {navigation.map((item) => (
                <a
                  key={item.id}
                  href={`#${item.id}`}
                  className={`topbar-pill ${activeView === item.id ? "active" : ""}`.trim()}
                  aria-current={activeView === item.id ? "page" : undefined}
                >
        ...
        <div className="topbar-actions">
          <div className="topbar-status-row">
            {workspaceSource === "supabase" ? (
              ...
              <span className="pill muted">Mock</span>
            )}
            {pwaStatusLabel ? <span className={`pill ${pwaStatusTone}`}>{pwaStatusLabel}</span> : null}
            {rankSummary ? <RankBadgePill rankSummary={rankSummary} compact /> : null}
            {isCompactShell && currentTrader?.displayName ? (
              <span className="topbar-user-chip compact">{currentTrader.displayName}</span>
            ) : null}
          </div>
          <div className={`topbar-action-row ${isCompactShell ? "compact-actions" : ""}`.trim()}>
            <button
              className={`button secondary notification-trigger ${unreadNotificationCount ? "is-attention" : ""}`.trim()}
              type="button"
              onClick={onToggleNotifications}
              aria-expanded={isNotificationPanelOpen}
              aria-controls="trade-ready-panel"
            >
              Trade Ready {unreadNotificationCount ? `(${unreadNotificationCount})` : ""}
            </button>
            {!isCompactShell ? (
            ...
            {!isCompactShell && canInstall ? (
              <button className="button tertiary" type="button" onClick={onInstallApp} disabled={installPending}>
                {installPending ? "Opening install..." : "Install App"}
              </button>
            ) : null}
            {!isCompactShell && currentTrader?.displayName ? <span className="topbar-user-chip">{currentTrader.displayName}</span> : null}
            {!isCompactShell && onSignOut ? (
              <button className="button tertiary" type="button" onClick={onSignOut}>
                Sign Out
              </button>
            ) : null}
...
function getTradeReadyEventMeta(notification) {
  const payload = notification?.actionPayload ?? {};

  return {
    notification_id: notification?.id ?? null,
    listing_id: payload.listingId ?? notification?.listingId ?? null,
    target_trader_id: payload.targetTraderId ?? notification?.sellerTraderId ?? null,
    give_item_id: payload.youGiveItemId ?? notification?.offeredItemId ?? null,
    get_item_id: payload.youGetItemId ?? notification?.requestedItemId ?? null,
    match_score: payload.matchScore ?? notification?.matchScore ?? null,
    fairness_score: payload.fairnessScore ?? notification?.fairnessScore ?? null,
  };
}

function NotificationPanel({ notifications, onStartTrade, onMarkRead }) {
  if (!notifications.length) {
    return (
      <section className="notification-panel" id="trade-ready-panel" aria-label="Trade-ready notifications">
        <div className="notification-panel-head">
          <div>
            <p className="eyebrow">Trade-ready panel</p>
            <h3>No active trade-ready alerts</h3>
          </div>
        </div>
      </section>
    );
  ...
  return (
    <section className="notification-panel" id="trade-ready-panel" aria-label="Trade-ready notifications">
      <div className="notification-panel-head">
        <div>
          <p className="eyebrow">Trade-ready panel</p>
          <h3>Start the cleanest trades first</h3>
        </div>
      </div>
      <div className="notification-list">
        {notifications.slice(0, 4).map((notification) => {
          const payload = notification.actionPayload ?? {};
          const sellerUsername = payload.sellerUsername || notification.sellerUsername || "trader";
          const giveName = payload.youGiveName || notification.offeredItemName || "your item";
          const getName = payload.youGetName || notification.requestedItemName || "their item";
          const matchScore = Math.round(Number(notification.matchScore ?? payload.matchScore ?? 0));
          const fairnessScore = Math.round(Number(notification.fairnessScore ?? payload.fairnessScore ?? 0));

          return (
            <article
              className={`notification-card ${notification.isRead ? "" : "is-unread"}`.trim()}
              key={notification.id}
            >
              ...
              <div className="notification-card-body">
                <div className="notification-card-topline">
                  <strong>{notification.title || "Trade Ready Now"}</strong>
                  <span>{notification.createdAt ? formatDateTime(notification.createdAt) : "Now"}</span>
                </div>
                <p>{notification.body || `A strong trade is available with @${sellerUsername}.`}</p>
                <div className="notification-pair-box">
                  <div className="notification-pair-line">
                    <span>You give</span>
                    <strong>{giveName}</strong>
                  </div>
                  <div className="notification-pair-line">
                    <span>You get</span>
                  ...
                  <div className="notification-chip-row">
                    <span className="pill accent">{matchScore}% match</span>
                    <span className="pill warm">{fairnessScore}% fair</span>
                  </div>
                </div>
                <div className="button-row compact notification-actions">
                  <button type="button" className="button primary" onClick={() => onStartTrade(notification)}>
                    Start Trade
                  </button>
                  {!notification.isRead ? (
                    <button
...
function GlobalMascotWidgets({ currentTrader }) {
  return (
    <>
      <div className="welcome-bubble" aria-hidden="true">
        <img src="/hkdv-redesign/ghost-mascot.png" alt="" loading="lazy" decoding="async" />
        <div>
          <strong>Welcome back!</strong>
          <span>{currentTrader?.displayName ? `${currentTrader.displayName}, ready to trade?` : "Ready to trade?"}</span>
        </div>
      </div>
  ...
  return (
    <div className="auth-shell">
      <section className="auth-surface">
        ...
        <p>
          Use the same email as your trader profile. On first login, the app links your Supabase user to the matching trader record.
        </p>
    ...
    <section className="live-setup-banner" aria-label="Live workspace setup">
      <div className="live-setup-copy">
        ...
        <div className="button-row start">
          <a className="button secondary" href={currentTrader?.isAdmin ? "#admin" : "#dashboard"}>
            {currentTrader?.isAdmin ? "Open Admin" : "Open Dashboard"}
          </a>
        </div>
      </div>
...
function DashboardView({
  ...
  tradeReadyCount,
  rankSummary,
  onCatalogActionComplete,
}) {
  const spotlightPlan = smartOpportunityRows[0] ?? null;
  const {
    matches: liveTradeMatches,
    loading: liveTradeMatchesLoading,
    error: liveTradeMatchesError,
  } = useTradeMatches(workspaceSource === "supabase" ? currentTrader?.id : null);

  return (
    <div className="view-stack">
      <HeroBanner
        eyebrow="HKDV Trade"
        title="Trade your doubles with the cutest collectors in the village."
        subtitle="Browse live listings, instant trade-ready matches, and wishlist opportunities in one soft-pink marketplace flow."
        ctaText="Start Trading"
        onCtaClick={() => {
          if (spotlightPlan?.listing?.id) {
            openOfferBuilder(spotlightPlan.listing.id);
          }
        }}
        secondaryText="Review Queue"
        secondaryHref="#offers"
        chips={[
          { label: `${tradeReadyCount} trade-ready`, tone: "accent" },
          { label: `${pendingIncomingCount} incoming`, tone: "warm" },
          { label: workspaceSource === "supabase" ? "Supabase live" : "Mock preview", tone: "muted" },
          ...(rankSummary ? [{ label: rankSummary.label, tone: rankSummary.tone }] : []),
        ]}
      />
      ...
      {workspaceSource === "supabase" ? (
        <CatalogSection trader={currentTrader} onActionComplete={onCatalogActionComplete} />
      ) : null}

      {workspaceSource === "supabase" ? <AssetManagerSection trader={currentTrader} /> : null}

      {workspaceSource === "supabase" ? (
        <SurfaceSection
          eyebrow="Mutual Trades"
          title="Live trade matches"
          description="Cross-referenced from your wishlist and duplicate inventory so you can act without hunting."
        >
          {liveTradeMatchesLoading ? <p className="catalog-state">Scanning the live market for mutual matches...</p> : null}
          {!liveTradeMatchesLoading && liveTradeMatchesError ? (
            <p className="catalog-state error">{liveTradeMatchesError}</p>
          ) : null}
          {!liveTradeMatchesLoading && !liveTradeMatchesError && liveTradeMatches.length ? (
            <div className="trade-match-grid">
              {liveTradeMatches.slice(0, 4).map((match) => (
                <TradeMatchCard key={match.otherTrader.id} match={match} />
              ))}
            </div>
          ) : null}
          {!liveTradeMatchesLoading && !liveTradeMatchesError && !liveTradeMatches.length ? (
            <p className="catalog-state">
              No mutual matches yet. Add more wishlist targets and duplicate inventory to improve the scan.
            </p>
          ) : null}
        </SurfaceSection>
      ) : null}

      {workspaceSource === "supabase" ? <DreamBoardSection trader={currentTrader} /> : null}
      ...
      <div className="content-grid two-up">
        <SurfaceSection
          ...
          description="The fastest paths back into the market, styled like the new HKDV marketplace home."
        >
...
function AdminView({
  ...
  const [itemsCatalogRows, setItemsCatalogRows] = useState([]);
  const [itemsCatalogStatus, setItemsCatalogStatus] = useState("loading");
  const [itemsCatalogError, setItemsCatalogError] = useState("");
  const [itemsCatalogTotalCount, setItemsCatalogTotalCount] = useState(0);
  ...
  useEffect(() => {
    ...
    setItemsCatalogStatus("loading");
    setItemsCatalogError("");

    import("./lib/itemsCatalogClient")
      .then(({ loadItemsCatalogWithSource }) => loadItemsCatalogWithSource())
      .then(({ rows, totalCount }) => {
        if (cancelled) {
          return;
        }

        setItemsCatalogRows(rows);
        setItemsCatalogTotalCount(totalCount);
        setItemsCatalogStatus("ready");
      })
      .catch((error) => {
        if (cancelled) {
          return;
        ...
        setItemsCatalogStatus("error");
        setItemsCatalogError(error.message || "Oops! Momo couldn't find the items.");
      });
  ...
  return (
    <div className="view-stack">
      ...
      <SurfaceSection
        eyebrow="Supabase Items"
        title="MomoMint Catalog"
        description="Live preview from the public.items table, adapted for this Vite React app."
      >
        {itemsCatalogStatus === "loading" ? (
          <p className="catalog-state">Loading MomoMint Catalog...</p>
        ) : null}

        {itemsCatalogStatus === "error" ? (
          <p className="catalog-state error">{itemsCatalogError}</p>
        ) : null}

        {itemsCatalogStatus === "ready" && !itemsCatalogRows.length ? (
          <p className="catalog-state">The catalog is empty! Time to add some cute things.</p>
        ) : null}

        {itemsCatalogStatus === "ready" && itemsCatalogRows.length ? (
          <>
            <div className="catalog-meta">
              <span>{formatNumber(itemsCatalogTotalCount)} live items</span>
              {itemsCatalogTotalCount > itemsCatalogRows.length ? (
                <span>Showing first {formatNumber(itemsCatalogRows.length)} rows</span>
              ) : null}
            </div>
            <div className="momomint-grid">
              {itemsCatalogRows.map((item) => (
                <article key={item.id} className="momomint-card">
                  <div className="momomint-card-head">
                    <strong>{item.name}</strong>
                    <StatusPill tone={toneForWikiRarity(item.wiki_rarity)}>{item.wiki_rarity}</StatusPill>
                  </div>
...
function App() {
  ...
  const [tradeReadyOfferContext, setTradeReadyOfferContext] = useState(null);
  ...
  const shownTradeReadyIdsRef = useRef(new Set());
  ...
  useEffect(() => {
    if (activeView !== "admin" || currentTrader?.isAdmin) {
      return;
    }

    setLiveMessage("Admin is only available from an admin-linked desk.");
  ...
  }, [activeView, currentTrader?.isAdmin]);
  ...
  const tradeReadyNotifications = useMemo(
    () =>
      notifications
        .filter((notification) => notification.type === "trade_ready")
        .sort(
          (left, right) =>
  ...
  const unreadTradeReadyCount = tradeReadyNotifications.filter((notification) => !notification.isRead).length;
  ...
  const navigation = useMemo(() => {
    ...
    return NAV_ITEMS.filter((item) => item.id !== "admin" || currentTrader?.isAdmin).map((item) => ({
      ...item,
  ...
  useEffect(() => {
    if (workspaceSource !== "supabase") {
      ...
      shownTradeReadyIdsRef.current.clear();
      return;
    }

    if (unreadTradeReadyCount > previousUnreadNotificationCountRef.current) {
      setIsNotificationPanelOpen(true);
    }

    previousUnreadNotificationCountRef.current = unreadTradeReadyCount;
  }, [unreadTradeReadyCount, workspaceSource]);

  useEffect(() => {
    if (workspaceSource !== "supabase" || !isNotificationPanelOpen) {
      return;
    }

    tradeReadyNotifications.forEach((notification) => {
      if (shownTradeReadyIdsRef.current.has(notification.id)) {
        return;
      }

      shownTradeReadyIdsRef.current.add(notification.id);
      void trackTradeEvent("trade_ready_shown", getTradeReadyEventMeta(notification));
    });
  }, [isNotificationPanelOpen, tradeReadyNotifications, workspaceSource]);
  ...
  function closeOfferBuilder() {
    ...
    setDraftOffer({});
    setTradeReadyOfferContext(null);
  }
  ...
  async function handleMarkNotificationRead(notificationId) {
    ...
    try {
      ...
      setWorkspaceWarning(error.message || "Unable to mark the notification as read.");
    }
  }

  async function openTradeReadyOffer(notification) {
    const payload = notification.actionPayload ?? {};
    const listingId = payload.listingId ?? notification.listingId ?? null;
    const offeredItemId = payload.youGiveItemId ?? notification.offeredItemId ?? null;
    const sellerUsername = payload.sellerUsername || notification.sellerUsername || "trader";
    const eventMeta = getTradeReadyEventMeta(notification);
    ...
    setOfferBuilderListingId(listingId);
    if (offeredItemId) {
      setDraftOffer({ [offeredItemId]: 1 });
    } else if (!loadSuggestedBundle(listingId)) {
      setDraftOffer({});
    }
    setTradeReadyOfferContext(eventMeta);
    setIsNotificationPanelOpen(false);
    setLiveMessage(`Trade-ready bundle loaded with @${sellerUsername}.`);
    setNotifications((current) =>
      current.map((entry) =>
    ...
    if (workspaceSource === "supabase") {
      try {
        await Promise.all([
          ...
          trackTradeEvent("trade_ready_clicked", eventMeta),
          trackTradeEvent("instant_trade_modal_opened", eventMeta),
        ]);
      } catch (error) {
        setWorkspaceWarning(error.message || "Unable to record the trade-ready action.");
      }
    }
  }
  ...
  async function handleCatalogActionComplete(action) {
    const successMessage =
      action?.type === "wishlist" ? "Wishlist updated." : "Inventory updated.";

    if (workspaceSource === "supabase") {
      try {
        await refreshSupabaseWorkspace(successMessage);
      } catch (error) {
        setWorkspaceWarning(
          error.message || "Unable to refresh the workspace after the catalog update."
        );
      }
  ...
  async function handleSendOffer() {
    ...
    const tradeReadyMeta = tradeReadyOfferContext
      ? {
          ...tradeReadyOfferContext,
          offer_item_count: selectedOfferItems.length,
          fairness_label: fairness.label === "neutral" ? "underpay" : fairness.label,
        }
      : null;
    ...
    if (workspaceSource === "supabase") {
      try {
        if (tradeReadyMeta) {
          await trackTradeEvent("instant_trade_confirm_clicked", tradeReadyMeta);
        }
        ...
        if (tradeReadyMeta) {
          await trackTradeEvent("offer_created", tradeReadyMeta);
        }
  ...
  switch (activeView) {
    ...
    case "trades":
      ...
      break;
    case "admin":
      viewBody = currentTrader?.isAdmin ? (
        <AdminView
        ...
        <SurfaceSection
          ...
          description="Sign in with an admin-linked trader to open moderation and seed controls."
        >
          <p className="catalog-state">
            Admin is only available from an admin-linked desk. The workspace has redirected non-admin access back to Dashboard.
          </p>
        </SurfaceSection>
      );
      break;
    ...
    default:
      viewBody = (
        <DashboardView
          ...
          tradeReadyCount={unreadTradeReadyCount}
          rankSummary={currentRankSummary}
          onCatalogActionComplete={handleCatalogActionComplete}
        />
      );
      break;
  }
  ...
  return (
    <>
      ...
      <div className={`app-shell ${isCompactShell ? "compact-shell" : ""}`.trim()}>
        ...
        <div className="main-shell">
          <TopBar
            ...
            pendingIncomingCount={pendingIncomingCount}
            unreadNotificationCount={unreadTradeReadyCount}
            isNotificationPanelOpen={isNotificationPanelOpen}
            onToggleNotifications={() => setIsNotificationPanelOpen((current) => !current)}
            ...
            rankSummary={currentRankSummary}
          />
          {workspaceSource === "supabase" && isNotificationPanelOpen ? (
            <NotificationPanel
              notifications={tradeReadyNotifications}
              onStartTrade={openTradeReadyOffer}
      ...
      <GlobalMascotWidgets currentTrader={currentTrader} />
      </>
      ) : null}
    </>
  );
